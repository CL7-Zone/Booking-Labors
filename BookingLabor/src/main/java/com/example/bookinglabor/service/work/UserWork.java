...
public class UserWork implements UserService {
    ...
    @Override
    public void createUserRole(Role role, String email) {
    ...
    @Override
    @Transactional
    public void deleteUserRole(Role role, String email) {

        UserAccount user = userRepository.findByEmailAndProvider(email, EnumComponent.SIMPLE);
        if(user != null){
            userRepository.deleteUserRole(role.getName(), user.getEmail());
        }
    }

    @Override
    public void updateUserRole(Role role, String email) {
